// Date page functionality - Reads date from URL parameter (?date=YYYY-MM-DD)
// DO NOT MODIFY - This file parses URL parameters
let currentDate = new Date();

// Initialize date page
function initializeDatePage() {
    // Load date from URL parameter or today's date
    loadDateFromURL();
...
if (document.readyState === 'complete') {
    ...
    window.addEventListener('load', initializeDatePage);
}

// Load date from URL parameter or default to today
...
// Load today's date (for compatibility)
function loadTodayDate() {
    loadDateFromURL();
}

// Setup date navigation buttons - Rest of file continues from today.js
...
// Load Muhurtam information for the selected date
async function loadTodayMuhurtam(dateStr) {
    try {
        ...
        if (response && response.muhurtam_data) {
            ...
            // Check each muhurtam type for this date
        ...
        console.error('Error loading muhurtam for date:', error);
    }
}
